...
export default function FindConnection() {
    ...
    console.log(con)
		if (con.rowCount > 0) {setConnections(con.rows);
    	setError("");}
		else {
		...
		<div>
			...
			{connections.length > 0 &&
				connections.map((connection: any) => (
					<div
						className="relative mt-4 flex w-full justify-between items-center bg-gray-300 min-h-24 rounded-md"
						key={connection.id}
					>
						<Typography className="absolute left-4">
							{connection.tickets_name}
						</Typography>
						<Typography className=" whitespace-pre flex-grow flex items-center justify-center">
							{' '}
							{connection.origin}{' '}
							<FlightIcon
								sx={{
            ...
            <Typography className='absolute right-4'>{connection.prices[4]}zł</Typography>
					</div>
				))}
		</div>
	);
